...
#define MPU6050							//֨ӥϒćʹԃքԫِǷΪMPU6050
#define MOTION_DRIVER_TARGET_MSP430		//֨ӥȽ֯ҿؖ,ӉԃMSP430քȽ֯(ӆֲսSTM32F4)
...
#define log_i 	printf	//ղӡхϢ
#define log_e  	printf	//ղӡхϢ
...
u8 mpu_dmp_init(void)
{
	...
	HAL_I2C_Init(&hi2c1);
	HAL_Delay(100);
	if(mpu_init()==0)	//ԵʼۯMPU6050
	{	 
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//ʨ׃̹ѨҪքԫِǷ
		if(res)return 1; 
		res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);//ʨ׃FIFO
		if(res)return 2; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//ʨ׃ӉҹÊ
		if(res)return 3; 
		res=dmp_load_motion_driver_firmware();		//ݓ՘dmpڌݾ
		if(res)return 4; 
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//ʨ׃ΓÝӇ׽в
		if(res)return 5; 
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//ʨ׃dmp٦Ŝ
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL);
		if(res)return 6; 
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//ʨ׃DMPˤԶ̙Ê(خճһӬڽ200Hz)
		if(res)return 7;   
		res=run_self_test();		//ؔݬ
		if(res)return 8;    
		res=mpu_set_dmp_state(1);	//ʹŜDMP
		if(res)return 9;     
	}
	return 0;
}
//փսdmpԦm۳ք˽ߝ(עӢ,Ѿگ˽ѨҪ҈ޏנבջ,ߖҿҤԐ֣נ)
//pitch:ةҶއ ޫ׈:0.1£   ׶Χ:-90.0£ <---> +90.0£
//roll:ۡڶއ  ޫ׈:0.1£   ׶Χ:-180.0£<---> +180.0£
//yaw:ڽвއ   ޫ׈:0.1£   ׶Χ:-180.0£<---> +180.0£
//׵ܘֵ:0,ֽӣ
//    Ǥ̻,ʧќ
#include <stdio.h>
